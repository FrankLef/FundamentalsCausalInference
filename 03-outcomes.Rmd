...
# hence the u in bootu.r, and their confidence intervals.
...
#    bootstrapping
# Assumptions: Weassume that (3.2) holds but not (3.1)
# See p. 45 and 46 for more details.
bootu.r <- function(dat, formula = Y ~ `T`, R = 1000, conf = 0.95) {
...
```{r}
# create a function for the table as it will used several times
tbl_bootc.r <- function(df, title = "Title", subtitle = "Subtitle") {
  df %>%
    gt() %>%
    tab_header(
    ...
    fmt_number(columns = !matches("name"), decimals = 2) %>%
...
startTime <- Sys.time()
# d <- bootu.r(dat = dataGSS, formula = trump ~ gthsedu)
a_file <- file.path(dir_data, "chap03_bootu.rds")
# saveRDS(d, file = a_file)
d <- readRDS(file = a_file)
endTime <- Sys.time()
...
tbl_bootc.r(d, 
            title = "Table 3.2", 
            subtitle = "Unconditional probabilities and association measures with 95% CI")
```
...
stopifnot(
  all(abs(d$est - bb$est) < 0.01),
  all(abs(d$lci - bb$lci) < 0.015),
  all(abs(d$uci - bb$uci) < 0.02))
```
...
**We therefore rename the `lmodboot.r()` of chapter 3 as `bootc.r`** to

* show it relates to `bootu.r` with the `c` meaning **conditional** and
...
bootc.r <- function(dat, formula = Y ~ `T` + A + H,
                    cond0 = Y ~ A + H,
                    cond1 = Y ~ `T` + A + H, 
...
```{r}
# we use cond = c("magthsedu", "white", "female", "gt65")
# to exclude gthsedu = 0
message("this takes 10 sec.: we load a saved file instead")
startTime <- Sys.time()
# d <- bootc.r(dat = dataGSS, 
...
tbl_bootc.r(d, 
            title = "Table 3.3",
            subtitle = "Cond. prob. and association measures with 95% CI")
```
...
# startTime <- Sys.time()
d1a <- bootu.r(dat = dataGSS1, formula = owngun ~ conservative)
...
```{r}
tbl_bootc.r(d1a[, c("name", "est", "lci", "uci")], 
...
```{r}
tbl_bootc.r(d1b[, c("name", "est", "lci", "uci")], 
            title = "Exercise 1 b): Table 3.2",
            subtitle = "Cond. prob. and association measures with 95% CI")
```
...
```{r}
tbl_bootc.r(d2[, c("name", "est", "lci", "uci")], 
...
# endTime <- Sys.time()
# print(endTime - startTime)
message("This took 26 min, so we load a saved file.")
...
```{r}
tbl_bootc.r(d3a[, c("name", "est", "lci", "uci")], 
...
```{r}
tbl_bootc.r(d3b[, c("name", "est", "lci", "uci")], 
            title = "Exercise 3 b)",
            subtitle = "Cond. prob. and association measures with 95% CI")
```
