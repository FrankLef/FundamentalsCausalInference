...
# p <- uncond %>%
#   filter(name %in% c("p0", "p1")) %>%
#   ggplot(mapping = aes(x = name, y = est, ymin = lci, ymax = uci, group = 1)) +
#   geom_linerange(size = 5, color = "gold") +
#   geom_point(shape = 19, size = 4, color = "mediumblue") +
#   geom_line(color = "mediumblue") +
#   ggrepel::geom_text_repel(aes(y = est, label = round(est, 3)), color = "mediumblue") +
#   ggrepel::geom_text_repel(aes(y = lci, label = round(lci, 3)), color = "mediumblue") +
#   ggrepel::geom_text_repel(aes(y = uci, label = round(uci, 3)), color = "mediumblue") +
#   theme_minimal() +
...
```{r eval=FALSE, include=FALSE}
# tbl %>%
#   gtsave("tbl_03_02.png", expand = 10, path = tempdir())
# pn <- png::readPNG(source = file.path(tempdir(), "tbl_03_02.png"))
# gridExtra::grid.arrange(p, grid::rasterGrob(pn), nrow = 1)
```
...
# endTime <- Sys.time()
# print(endTime - startTime)
message("This took 15 min, so we load a saved file.")
