...
population $p(1-p)$ which is not entirely the right way since the standard error
of the mean is defined using the *standard deviation of the sample*.
...
whatifdat.fnc <- function(data, ids, formula = Y ~ `T` + A + H) {
  dat <- data[ids, ]
  prob_lmod(data = dat, formula = formula)
}
...
sapply(X = seq_along(whatifdat.boot$t0), FUN = function(i, alpha = 0.05, method = "norm") {
    est <- whatifdat.boot$t0[i]
    ci <- boot::boot.ci(whatifdat.boot, conf = 1 - alpha, type = method, 
                        index = i)$normal
    c(".lower" = plogis(ci[2]),
...
To be fully tidyverse-like we recode the `prob_lmod` function from above and
call it `prob_lmod_td` where the `td` suffix stands for `tidyverse`. Note the
use of a formula as an argument to the function and the `rlang` package which
is a great tool to learn for quality code in R.
...
We obtain the confidence interval using `rsample::int_pctl` which uses
...
#                 inv = "expit", evars = "logit", formula = Y ~ `T` + A + H)
# endTime <- Sys.time()
