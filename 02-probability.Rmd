...
load(file.path(a_dir, "whatifdat.RData"))
...
This code reflects better coding practices than the textbook's I think.
In particular with the use of `replicate()` and avoiding the `for` loop.

```{r}
sim.r <- function(nsim = 1e4, n = 1e4) {
...
The textbook's version of this function has 13 lines.  This one only has 7
and use `boot::boot.ci()` and `stats::plogis()` to simplify.

Also we change the way the function works to

* harmonize it with other similar functions coming up in later chapters
* use formulas in the parameters to better understand what the process is. 
This point is important as this is, in my experience, the tricky part to 
understand.


```{r}
#  - Estimate the unconditional sampling distribution when using all
#    variables with cond = all.vars(formula[[3]]).
# - It is the conditional sampling distribution when using only some predictors
#    such as with Y ~ `T` + A + H as is done in exercise 5 below.
lmodboot.r <- function(dat, formula = Y ~ `T` + A + H,
                       cond = Y ~ `T` + A + H, 
                       R = 1000, conf = 0.95) {
  ...
  out <- c("est" = plogis(est.logit), "conf" = ci.logit[1], 
...
```{r}
parametric <- lmodboot.r(whatifdat, formula = Y ~ `T` + A + H, 
                         cond = Y ~ `T` + A)
parametric
```
