...
`coefficient()`. The example in the book uses `lmod$coef` which is a bad
coding practice.  The data should be obtained with an extractor function such as
`coef()`.
...
```
and we can validate the result directly with a nonparametric estimate using
the dataset
