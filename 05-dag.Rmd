...
```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.1", warning=FALSE, out.width="60%"}
scm_5.1$plot
```
...
```{r fig.align='center', fig.cap="Figure 5.2", warning=FALSE}
...
```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.3", warning=FALSE, out.width="60%"}
scm_5.3$plot
```
...
```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.4", warning=FALSE, out.width="60%"}
scm_5.4$plot
```
...
scm_5.5 <- within(scm_5.5, {
  ...
  plot <- dag %>% 
    ...
    geom_dag_edges() +
    geom_dag_text(size = 7, color = "midnightblue") +
    scale_color_manual(values = status_colrs, na.value = colr) +
    ggdag::theme_dag(panel.background = element_rect(fill = colr, color = colr)) +
...
```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.6", warning=FALSE, out.width="60%"}
scm_5.6$plot
```
...
```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.7", warning=FALSE, out.width="60%"}
scm_5.7$plot
```
...
```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.8", warning=FALSE, out.width="60%"}
scm_5.8$plot
```
...
and the causal DAG fortthe What-If? study is


```{r echo=TRUE, fig.align='center', fig.cap="Figure 5.9", warning=FALSE, out.width="80%"}
scm_5.9 <- list()
scm_5.9 <- within(scm_5.9, {
  ...
  plot <- dag %>% 
    ...
    geom_dag_text(size = 5, color = "midnightblue" ,
                  parse = TRUE,
                  label = c("A", expression(AD[0]), expression(AD[1]),
                            "T", "U", expression(VL[0]), expression(VL[1]))) +
    scale_color_manual(values = status_colrs, na.value = colr) +
    ggdag::theme_dag_blank(panel.background = 
                             element_rect(fill=colr, color=colr)) +
    theme(legend.position = "bottom",
          legend.title = element_blank()) +
...
```{r echo=TRUE, warning=FALSE}
scm_5.10 <- list()
scm_5.10 <- within(scm_5.10, {
    the_nodes <- c("A" = "Weight",
                   "W1" = "Exercise",
                   "W2" = "Strength",
                   "W3" = "Diet",
                   "Y" = "Nutrient")
  ...
  plot <- dag %>% 
    ...
    theme(legend.position = "bottom",
          legend.title = element_blank()) +
    labs(title = "Causal DAG for Exercise 1")
})
```
...
```{r echo=FALSE, fig.align='center', fig.cap="Figure 5.10", warning=FALSE, out.width="80%"}
scm_5.10$plot
```
...
$$
Strength \perp\!\!\!\perp Nutrient \mid Diet
$$

The we should conditioned on $Diet$ an the regression formula to determine
the effect of nutrient intake on increased strength would be to avoid the
confounding effect

$$
Strength = \beta_0 + \beta_1 Nutrient + \beta_2 Diet
$$
...
* The $exercise$ is a confounder and the analysis fail to take this into account
* $weight$ also has an effect unless we condition on $exercise$
...
scm_5.11 <- within(scm_5.11, {
  ...
  plot <- dag %>% 
    ...
    geom_dag_text(size = 7, color = "midnightblue" ,
                  parse = TRUE,
                  label = c("A", expression(H[1]), expression(H[2]),
                            expression(H[3]), expression(H[4]), "Y")) +
    scale_color_manual(values = status_colrs, na.value = colr) +
    ggdag::theme_dag(panel.background = element_rect(fill = colr, color = colr)) +
    theme(legend.position = "bottom",
          legend.title = element_blank()) +
    labs(title = "Causal DAG for Exercise 2")
})
```



```{r echo=FALSE, fig.align='center', fig.cap="Figure 5.11", warning=FALSE, out.width="80%"}
scm_5.11$plot
```
...
```{r}
adjustmentSets(scm_5.11$dag, exposure = "A", outcome = "Y")
```
