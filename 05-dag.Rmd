...
scm_5.1 <- within(scm_5.1, {
  ...
  plot <- dag %>% 
    ...
    geom_dag_label_repel(aes(label = label), col = text_colr) +
    scale_color_manual(values = status_colrs, na.value = bg_colr, guide = "none") +
    ggdag::theme_dag(panel.background = element_rect(fill = bg_colr, color = bg_colr))
})
```
...
scm_5.2 <- within(scm_5.2, {
  ...
  plotChain <- dagChain %>% 
    ...
    arrange(name) %>%  # must sort by name to use mathematical expressions
    ggdag() +
    geom_dag_point(color = bg_colr) +
    geom_dag_edges() +
    geom_dag_text(size = 5, color = text_colr,
                  parse = TRUE,
...
```{r echo=TRUE, fig.align='center', fig.cap="A Counfounder May Occur After the Exposure", warning=FALSE, out.width="60%"}
scm_5.5$plot
```
...
scm_5.6 <- within(scm_5.6, {
  ...
  plot <- dag %>% 
    ...
    geom_dag_label_repel(aes(label = label), color = text_colr, fill = label_colr,
                         box.padding = 1) +
    geom_dag_text(size = 5, color = text_colr) +
    scale_color_manual(values = status_colrs, na.value = bg_colr, guide = "none") +
    ggdag::theme_dag(panel.background = element_rect(fill = bg_colr, color = bg_colr))
...
scm_5.8 <- within(scm_5.8, {
  ...
  plot <- dag %>% 
    ...
    arrange(name) %>%
    ggdag_status(color = status, text = FALSE) +
    geom_dag_label_repel(aes(label = label), color = text_colr, fill = bg_colr,
                         box.padding = 1) +
    geom_dag_text(size = 5, color = text_colr) +
    scale_color_manual(values = status_colrs, na.value = bg_colr, guide = "none") +
    ggdag::theme_dag(panel.background = element_rect(fill = bg_colr, color = bg_colr))
...
scm_ex5.2po <- within(scm_ex5.2po, {
  ...
  plot <- dag %>% 
    ...
    geom_dag_point(aes(color = status)) +
    geom_dag_edges() +
    geom_dag_label_repel(aes(label = label), color = "midnightblue", fill = "oldlace",
                         box.padding = 1) +
...
scm_ex5.3 <- within(scm_ex5.3, {
  ...
  # format the tableau
  summ <- tbl_probs(tbl, title = "Table exercise 5.3",
