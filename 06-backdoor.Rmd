...
A plot can however illustrate well Brumback's main points.
...
rbind(
  ...
  mutate(lci = mean - 2 * sd,
         uci = mean + 2 * sd) %>%
  mutate(across(.cols = c("mean", "lci", "uci"), .fns = round, digits = 2)) %>%
  ggplot(aes(x = mean, xmin = lci, xmax = uci,
             y = estimator, color = simulation)) +
  geom_pointrange(position = position_dodge(width = 0.5)) +
  ggrepel::geom_text_repel(aes(x = mean, y = estimator, label = mean), size = 3) +
  ggrepel::geom_text_repel(aes(x = lci, y = estimator, label = lci), size = 3) +
  ggrepel::geom_text_repel(aes(x = uci, y = estimator, label = uci), size = 3) +
  theme_minimal() +
  ...
  labs(title = "Chap 6, section 6.3: Simulation of Standardization Methods",
       subtitle =
         sprintf("The mean with 95%% CI. Sample size = %d.", nrep),
       x = NULL, y = NULL)
```
