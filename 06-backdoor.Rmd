...
#        local = knitr::knit_global())
# source(file = file.path(dir_lib, "gt_measures_colgrp.R"),
#        local = knitr::knit_global())
...
it means that we condition the data on $H$ and we set the $T$ column to $T = t$.
In the notation of causal inference we would write 
$E(Y=t) \mid T = t, H = h) = E(Y) \mid do(T = t), H = h)$.
:::
...
To compute the standardized estimate using outcome-model standardization
in a non-parametric way we use the function `fci::backdr_out_np()` or its alias
`fciR::stand()`.
...
whatif.out <- fciR::backdr_out_np(whatifdat, formula = Y ~ A + H + A:H)
whatif.out
```
...
```{r}
doublewhatif.out <- fciR::backdr_out_np(doublewhatifdat, formula = VL1 ~ A + AD0 + A*AD0)
```
...
```{r}
doublewhatif.out <- fciR::backdr_out_np(doublewhatifdat, formula = VL1 ~ A + VL0 + A*VL0)
```
...
```{r}
whatif.att <- fciR::backdr_out_np(whatifdat, formula = Y ~ A + H + A*H, att = TRUE)
```
...
```{r}
doublewhatif.att <- fciR::backdr_out_np(doublewhatifdat, formula = VL1 ~ A + AD0 + A*AD0,
                              att = TRUE)
```
...
```{r}
doublewhatif.att <- fciR::backdr_out_np(doublewhatifdat, formula = VL1 ~ A + VL0 + A*VL0,
                              att = TRUE)
```
...
```{r}
mortdat.out <- fciR::backdr_exp_np(mortdat, formula = Y ~ `T` + H, weights = "n")
...
tbl <- gt::gt(dft) %>%
  fciR::gt_basic(title = title, subtitle = subtitle) %>%
