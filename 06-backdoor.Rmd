...
and we can see it gives the same results with the `fciR` package with
`fciR::backdr_out_np`)

```{r}
message("This takes about 2 min. Load from file.")
# startTime <- Sys.time()
# mort.out.np <- fciR::boot_est(
#   mort, fciR::backdr_out_np, times = 500, alpha = 0.05, transf = "exp", 
...
```{r}
message("This takes about 4 min. Load from file.")
# startTime <- Sys.time()
...
mort.out.sat <- readRDS(file = a_file)
```
...
whatif.out.sat <- readRDS(file = a_file)
```
...
```{r}
message("This takes about 20 sec.. Load from file.")
# startTime <- Sys.time()
...
and the results are the same with both non-parametric and with the saturated 
model.
...
and the results are the same wether one uses non-parametric or a fully saturated
regression model.
...
whatif.out.att.sat <- readRDS(file = a_file)
```
...
doublewhatif.out.att.np <- readRDS(file = a_file)
```
...
message("This takes about 1 sec.. Load from file.")
# startTime <- Sys.time()
...
whatif2.out <- readRDS(file = a_file)
```
...
```{r}
message("This takes about 2 sec.. Load from file.")
startTime <- Sys.time()
...
mortdat.exp.np <- readRDS(file = a_file)
```
...
mortdat.exp.np.att <- readRDS(file = a_file)
```
...
```{r}
message("This takes about 6 sec.. Load from file.")
# startTime <- Sys.time()
...
whatif2.exp.gee <- readRDS(a_file)
```
...
whatif2.bad <- readRDS(file = a_file)
```
