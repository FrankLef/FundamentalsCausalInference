...
```{r}
brfss4cond <- brfss4[brfss4$rural == 1, ]
PZ1R1a <- sum(brfss4cond$maxdrinks == 1) / nrow(brfss4cond)

mod.logit <- glm(formula = zerodrinks ~ rural + gt65 + female + whitenh + blacknh +
                   hisp + multinh + gthsedu, 
                 family = "binomial", data = brfss4)
PZ1R1b <- mean(fitted(mod.logit))

# compute probabilities from coefficients
PZ1R1coefs <- coef(mod.logit)
PZ1R1c <- sum(PZ1R1coefs)
PZ1R1c <- plogis(PZ1R1c)


# create the data where the probability assumes each individual is rural
brfss_PZ1R1 <- brfss4
brfss_PZ1R1$rural <- 1
mod.logit.predict_PZ1R1 <- predict(mod.logit, newdata = brfss_PZ1R1, type = "response")
PZ1R1d <- mean(mod.logit.predict_PZ1R1)
...
# final
PZ1R1 <- PZ1R1a  # my answer
the_PZ1R1 <- c("PZ1R1a" = PZ1R1a, "PZ1R1b" = PZ1R1b, "PZ1R1c" = PZ1R1c, "PZ1R1d" = PZ1R1d)
...
# create the data for each individual if they consumed drinks and are in rural area
brfss_EYZ0 <- brfss4
brfss_EYZ0$zerodrinks <- 0
brfss_EYZ0R1 <- brfss_EYZ0
brfss_EYZ0R1$rural <- 1
mod.log.predict_EYZ0R1 <- predict(mod.log, newdata = brfss_EYZ0R1, type = "response")
EYZ0R1a <- mean(mod.log.predict_EYZ0R1)

EYZ0R1coefs <- coef(mod.log)
EYZ0R1b <- sum(EYZ0R1coefs[names(EYZ0R1coefs) != "zerodrinks"])
EYZ0R1b <- exp(EYZ0R1b)


# create data for the individuals assuming they are in rural
brfss_EYR1 <- brfss
brfss_EYR1$rural <- 1
mod.log.predict_EYR1 <- predict(mod.log, newdata = brfss_EYR1, type = "response")
EYZ0R1c <- mean(mod.log.predict_EYR1)

# final
EYZ0R1 <- EYZ0R1a  # my answer
the_EYZ0R1 <- c("EYZ0R1a" = EYZ0R1a, "EYZ0R1b" = EYZ0R1b, "EYZ0R1c" = EYZ0R1c)
...
```{r}
df <- expand.grid("PZ1R1" = the_PZ1R1, "EYZ0R1" = the_EYZ0R1, "EH" = c(EH, 1))
df <- df %>%
  mutate(EY = EH * (EYZ0R1 * (1 - PZ1R1))) %>%
  arrange(desc(EY))
df
```
