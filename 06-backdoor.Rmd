...
1 line per thousands of observations.


```{r}
mort <- mortality %>%
  select(H, `T`, Y, n) %>%
  mutate(n = as.integer(n / 1000)) %>%
  uncount(n)
str(mort)
```
...
We verify the algorithm once more using the `whatifdat` data set.


```{r}
data("whatifdat", package = "fciR")
whatif.out.est <- est_out_np(whatifdat, outcome = Y, exposure = `T`, confound = H)
whatif.out.est
```
and using the package we have the same result

```{r}
message("This takes about 15 sec.. Load from file.")
# startTime <- Sys.time()
# whatif.out.np <- fciR::backdr_out_np(whatifdat, outcome = Y, exposure = `T`, confound = H,
#                                      R = 500)
# endTime <- Sys.time()
# print(endTime - startTime)
