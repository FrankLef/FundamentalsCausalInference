...
```{r}
rm(dataBRFSS)  # make sure to reset dataBRFSS
dataBRFSS <- brfss
```
...
mod.logit <- glm(formula = zerodrinks ~ rural + gt65 + female + whitenh + blacknh +
                 ...
                 family = "binomial", data = dataBRFSS)
# PZ1R1 <- mean(fitted(mod.logit))

# create the data where the probability assumes each individual is rural
dataBRFSS_PZ1R1 <- dataBRFSS
dataBRFSS_PZ1R1$rural <- 1
mod.logit.predict_PZ1R1 <- predict(mod.logit, newdata = dataBRFSS_PZ1R1, type = "response")
PZ1R1 <- mean(mod.logit.predict_PZ1R1)
PZ1R1
```
...
mod.log <- glm(formula = maxdrinks ~ rural + gt65 + female + whitenh + blacknh +
               ...
               family = "poisson", data = dataBRFSS)

# create the data for each individual if they consumed drinks and are in rural area
dataBRFSS_EYZ0 <- dataBRFSS
dataBRFSS_EYZ0$zerodrinks <- 0
dataBRFSS_EYZ0R1 <- dataBRFSS_EYZ0
dataBRFSS_EYZ0R1$rural <- 1
mod.log.predict_EYZ0R1 <- predict(mod.log, newdata = dataBRFSS_EYZ0R1, type = "response")
EYZ0R1 <- mean(mod.log.predict_EYZ0R1)

# coefs <- coef(mod.log)
# EYZ0R1 <- sum(coefs[names(coefs) != "zerodrinks"])
# EYZ0R1 <- exp(EYZ0R1)
# EYZ0R1


# create data for the individuals assuming they are in rural
# dataBRFSS_EYR1 <- dataBRFSS
# dataBRFSS_EYR1$rural <- 1
# mod.log.predict_EYR1 <- predict(mod.log, newdata = dataBRFSS_EYR1, type = "response")
# EYZ0R1 <- mean(mod.log.predict_EYR1)
...
```{r}
# mean(fitted(mod.log)) is the same as mean(dataBRFSS$maxdrinks)
