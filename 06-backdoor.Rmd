...
```{r}
mc.plot <- list()
mc.plot <- within(mc.plot, {
  df <- rbind(
    data.frame(model = "ss=40", mc.stats40),
    data.frame(model = "ss=100", mc.stats100)) %>%
    mutate(model = factor(model))
    
  p <- ggplot(df, aes(x = mean, xmin = lower, xmax = upper,
                         y = estimate, color = model)) +
    geom_pointrange(position = position_dodge(width = 0.5)) +
    ggrepel::geom_text_repel(aes(x = mean, y = estimate, label = round(mean, 3)),
                             size = 3) +
    ggrepel::geom_text_repel(aes(x = lower, y = estimate, label = round(lower, 3)),
                             size = 3) +
    ggrepel::geom_text_repel(aes(x = upper, y = estimate, label = round(upper, 3)),
                             size = 3) +
    scale_color_manual(values = c("ss=40" = "mediumseagreen", "ss=100" = "tomato")) +
    theme_minimal() +
    theme(legend.position = "bottom",
          legend.title = element_blank()) +
    labs(title = "Chap 6, section 6.3",
         subtitle = "Simulation of Standardization Methods",
         x = NULL, y = NULL)
})
mc.plot$p
```
