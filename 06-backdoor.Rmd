...
source(file = file.path(dir_lib, "fci_06-E_standexpgee.R"), 
       local = knitr::knit_global())
source(file = file.path(dir_lib, "fci_06-F_standdr.R"), 
       local = knitr::knit_global())
source(file = file.path(dir_lib, "fci_06-G_badstanddr.R"), 
       local = knitr::knit_global())
...
```{r file="lib\\fci_06-G_badstanddr.R"}
...
```{r}
message("This took about 3 min, so we load a saved file.")
# startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# brfss.out <- standout(brfss1, formula = flushot ~ insured + female + blacknh +
...
gt_measures(brfss.out, 
            ...
            subtitle = paste0("2019 Behavioral Risk Factor Surveillance System Survey", 
                              ...
                              "<br>",
                              "Effect of <em>insured</em> on <em>flushot</em>"))
```
...
stopifnot(sum(abs(comp$sim - comp$bb)) < 0.5)
```

**Assumptions**

* Outcome consistence: Seems ok here, getting a flu shot is well defined.
This could change overtime however and the survey is from 2019.
* Positivity assumption of *insured*. Maybe some individuals have a zero
probability ans some other 1 but they would not be part of the study
* Sufficient confounder assumption: $female$ an$ethnicity$ sem to be sufficient 
confounders as required in (6.1) p. 99.
...
```{r}
message("This took 3 min, so we load a saved file.")
# startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# brfss.exp <- standexp(brfss1, formula = flushot ~ insured + female + blacknh +
#                hisp + multinh + gthsedu + rural, R = 100)
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap06_Ex2.rds")
# saveRDS(brfss.exp, file = a_file)
brfss.exp <- readRDS(a_file)
```
...
```{r}
message("This took 5 min, so we load a saved file.")
# startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# brfss.dr <- standdr(brfss1, formula = flushot ~ insured + female + blacknh +
#                hisp + multinh + gthsedu + rural, R = 100)
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap06_Ex3.rds")
# saveRDS(brfss.dr, file = a_file)
brfss.dr <- readRDS(a_file)
```
...
We have reduced $E(Y \mid R, H)$ to

$$
\begin{align*}
E(Y \mid R = r, H) &= E(Y \mid Z = 0, R = r, H) [1 - P(Z=1 \mid R = r, H)]
\end{align*}
$$
We will compute $E(Y \mid R = 1, H)$


$$
\begin{align*}
E(Y \mid R = 1, H) &= E(Y \mid Z = 0, R = 1, H) [1 - P(Z=1 \mid R = 1, H)]
\end{align*}
$$


We create different data subset to make the calculations

```{r}
brfss4_do_R1 <- brfss4
brfss4_do_R1$rural <- 1
brfss4_cond_R1 <- brfss4[brfss4$rural == 1, ]
brfss4_cond_Z0 <- brfss4[brfss4$zerodrinks == 0, ]
brfss4_cond_Z1 <- brfss4[brfss4$zerodrinks == 1, ]
brfss4_cond_Z0_do_R1 <- brfss4_cond_Z0
brfss4_cond_Z0_do_R1$rural <- 1
brfss4_cond_Z1_do_R1 <- brfss4_cond_Z1
brfss4_cond_Z1_do_R1$rural <- 1
brfss4_do_Z0_do_R1 <- brfss4
brfss4_do_Z0_do_R1$zerodrinks <- 0
brfss4_do_Z0_do_R1$rural <- 1
```


Now, since *The logistic parametric model is a model for $P(Z = 1 \mid R = r, H)$*
then

```{r}
prob_Z1 <- sum(brfss4$zerodrinks == 1) / nrow(brfss4)

prob_Z1_cond_R1 <- sum(brfss4_cond_R1$zerodrinks == 1) / nrow(brfss4_cond_R1)
# prob_Z1_cond_R1

# logistic model with data conditional on R=1
mod_logit_cond_R1 <- glm(formula = zerodrinks ~ rural + gt65 + female + whitenh + blacknh +
                   hisp + multinh + gthsedu, 
                 family = "binomial", data = brfss4_cond_R1)
mod_logit_cond_R1_mean <- mean(fitted(mod_logit_cond_R1))
# mod_logit_cond_R1_mean

# they should be very close
stopifnot(abs(prob_Z1_cond_R1 - mod_logit_cond_R1_mean) < 1e-6)


# final
the_PZ1 <- c("prob_Z1" = prob_Z1,
             "prob_Z1_cond_R1" = prob_Z1_cond_R1)
the_PZ1
```

and since *the loglinear parametric model is a model for 
$E(Y \mid Z = 0, R = r, H)$* then

```{r}
# This is the outcome-model standardization making sure that we predict
# for everyone in the sample, not just those with Z = 0
# It should be the right one !!
mod.log <- glm(formula = maxdrinks ~ rural + gt65 + female + whitenh + blacknh +
                 hisp + multinh + gthsedu + zerodrinks, 
               family = "poisson", data = brfss4)
mod.log.predict_do_Z0_do_R1 <- predict(mod.log, newdata = brfss4_do_Z0_do_R1, 
                                       type = "response")
EY_do_Z0_do_R1 <- mean(mod.log.predict_do_Z0_do_R1)


# a trial . . .
mod.log.predict_do_R1 <- predict(mod.log, newdata = brfss4_do_R1, type = "response")
EY_do_R1 <- mean(mod.log.predict_do_R1)
EY_do_R1


# Thi is only for the with Z = 0, should not be the correct answer!
mod.log_cond_Z0 <- glm(formula = maxdrinks ~ rural + gt65 + female + whitenh + blacknh +
                 hisp + multinh + gthsedu + zerodrinks, 
               family = "poisson", data = brfss4_cond_Z0)
mod.log.predict_cond_Z0_do_R1 <- predict(mod.log, newdata = brfss4_cond_Z0_do_R1, 
                                       type = "response")
EY_cond_Z0_do_R1 <- mean(mod.log.predict_cond_Z0_do_R1)


# output
the_EYZ0 <- c("EY_do_Z0_do_R1" = EY_do_Z0_do_R1, 
              "EY_cond_Z0_do_R1" = EY_cond_Z0_do_R1,
              "EY_do_R1" = EY_do_R1)
the_EYZ0
```

and the outer product of $E_H [E(Y \mid Z = 0, R = r, H) (1 - P(Z=1 \mid R = r, H))]$
is

```{r}
# mean(fitted(mod.log)) is the same as mean(brfss$maxdrinks)
EH <- mean(fitted(mod.log))
EH
```
...
and trying all variations does not help


```{r}
df <- expand.grid("PZ1" = the_PZ1, "EYZ0" = the_EYZ0, "EH" = c(EH, 1))
df <- df %>%
  mutate(EY = EYZ0 * (1 - PZ1),
         EHEY = EH * EY) %>%
  arrange(desc(EHEY))
df
```
...
Now using the outcome-model standardization. Note that we use the argument
`family = poisson` since the outcome is a count.  However we obtained the
same results using `family = gaussian`, actually, when using `family = gaussian`,
the results matched *exactly* with the exposure model of exercise 5 below.

```{r}
message("This took 3 min, so we load a saved file.")
startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# maxdrinks.out <- standout(dataBRFSS, formula = maxdrinks ~ rural + gt65 + female +
#                             blacknh + hisp + multinh + gthsedu, 
#                           family = "poisson", R = 50, conf = 0.95)
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap06_Ex4.rds")
# saveRDS(maxdrinks.out, file = a_file)
maxdrinks.out <- readRDS(a_file)
```
...
```{r}
gt_measures(maxdrinks.out, 
            title = "Exercise 6.4", 
            subtitle = paste0("2019 Behavioral Risk Factor Surveillance System Survey", 
                              "<br>", 
                              "Outcome-model Standardization", 
                              "<br>",
                              "Effect of <em>rural</em> on <em>maxdrinks</em>"))
```
...
With the exposure model we have

```{r}
message("This took 3 min, so we load a saved file.")
# startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# maxdrinks.exp <- standexp(dataBRFSS, formula = maxdrinks ~ rural + gt65 + female +
#                             blacknh + hisp + multinh + gthsedu,
#                           R = 50, conf = 0.95)
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap06_Ex5.rds")
# saveRDS(maxdrinks.exp, file = a_file)
maxdrinks.exp <- readRDS(a_file)
```


```{r}
gt_measures(maxdrinks.exp, 
            title = "Exercise 6.5", 
            subtitle = paste0("2019 Behavioral Risk Factor Surveillance System Survey", 
                              "<br>", 
...
With the doubly-robust model we have

```{r}
message("This took 5 min, so we load a saved file.")
startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# maxdrinks.dr <- standdr(dataBRFSS, formula = maxdrinks ~ rural + gt65 + female +
#                             blacknh + hisp + multinh + gthsedu,
#                         family = "poisson", R = 50, conf = 0.95)
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap06_Ex6.rds")
# saveRDS(maxdrinks.dr, file = a_file)
maxdrinks.dr <- readRDS(a_file)
```


```{r}
gt_measures(maxdrinks.dr, 
            title = "Exercise 6.7", 
            subtitle = paste0("2019 Behavioral Risk Factor Surveillance System Survey", 
                              "<br>", 
                              "Doubly-Robust Standardization", 
...
Computing the average treatment on the treatment (with `att = TRUE`), 
i.e. for rural county, with the exposure model 

```{r}
message("This took 3 min, so we load a saved file.")
startTime <- Sys.time()
# whitenh not included in formula since it is the reference dummy variable
# maxdrinks.expatt <- standexp(dataBRFSS, formula = maxdrinks ~ rural + gt65 + 
#                                female + blacknh + hisp + multinh + gthsedu, 
#                              att = TRUE, R = 50, conf = 0.95)
# endTime <- Sys.time()
# print(endTime - startTime)
# a_file <- file.path(dir_data, "chap06_Ex7.rds")
# saveRDS(maxdrinks.expatt, file = a_file)
# maxdrinks.expatt <- readRDS(a_file)
```


```{r}
# gt_measures(maxdrinks.expatt, 
#             title = "Exercise 6.7", 
#             subtitle = paste0("2019 Behavioral Risk Factor Surveillance System Survey", 
#                               "<br>", 
#                               "Exposure-model Standardization for the ATT", 
#                               "<br>",
#                               "Effect of <em>rural</em> on <em>maxdrinks</em>"))
```
