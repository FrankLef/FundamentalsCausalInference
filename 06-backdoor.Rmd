...
The simulaiton with 40 covariates
...
```{r}
# IMPORTANT: sample size not the same as
...
and we verify the p-values. Differences are caused by rounded numbers.
...
bb40 %>%
  mutate(`T` = ifelse(grepl(pattern = "0", estimator), 0, 1),
         ...
         n = 1000,
         sdm = sd / sqrt(n),
         z = abs(mean - h0) / sdm,  # how many sd from the mean
         pval = 2 * (1 - pnorm(q = z)),
         pval = round(pval, 3)) %>%
  select(-sdm, -z) %>%
...
and we verify the p-values. The differences are caused by rounded numbers.
...
and we can plot the results to facilitate the discussion

```{r}
rbind(
  data.frame(model = "ss=40", mc.stats40),
  data.frame(model = "ss=100", mc.stats100)) %>%
  mutate(across(.cols = c(mean, lower, upper), .fns = round, digits = 3)) %>%
  ggplot(aes(x = mean, xmin = lower, xmax = upper,
             y = estimator, color = model)) +
  geom_pointrange(position = position_dodge(width = 0.5)) +
  ggrepel::geom_text_repel(aes(x = mean, y = estimator, label = mean), 
                           size = 3) +
  ggrepel::geom_text_repel(aes(x = lower, y = estimator, label = lower),
                           size = 3) +
  ggrepel::geom_text_repel(aes(x = upper, y = estimator, label = upper),
                           size = 3) +
  scale_color_manual(values = c("ss=40" = "mediumseagreen", "ss=100" = "tomato")) +
  theme_minimal() +
  theme(title = element_text(color = "midnightblue"),
        legend.position = "bottom",
        legend.title = element_blank()) +
  labs(title = "Chap 6, section 6.3: Simulation of Standardization Methods",
       subtitle = "The mean with the 5% and 95% quantiles.",
       x = NULL, y = NULL)
```
