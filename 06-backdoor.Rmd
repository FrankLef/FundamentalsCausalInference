...
We use a sample size of only 500 vs the book's 1000.  We obtain the same 
estimates with,  actually, a lesser sd. See somments below.
...
The simulation with 40 covariates


```{r}
message("This takes about 1 min. Load from file.")
# startTime <- Sys.time()
# We only use 500 rather than the 1000 suggested in the book
# IMPORTANT: Will be used again whaen calculating p-value
# It gives the same results with actually less sd for EY1exp and EY1dr (!)
# mc.out40 <- mc_standdr(ss = c(40), nrep = nrep)
a_file <- file.path(dir_data, "chap06_mc_out40.rds")
mc.out40 <- readRDS(file = a_file)
# saveRDS(mc.out40, file = a_file)
# endTime <- Sys.time()
...
mc.stats40 <- mc.out40 %>%
  ...
  mutate(`T` = ifelse(grepl(pattern = "0", estimator), 0, 1),
         ...
         n = nrep,  # sample size not the same as the book's
         sdp = sd / sqrt(n),
         z = abs((mean - h0) / sdp),
         pval = 2 * (1 - pnorm(z)),
         pval = round(pval, 3)) %>%
  select(-sdp, -z) %>%
  relocate(estimator, `T`)
mc.stats40
```
and we compare with the author's
...
```{r}
bb40 <- data.frame(
  estimator = c("EYT0", "EYT1", "EY0exp", "EY1exp", "EY0exp2", "EY1exp2",
                "EY0out", "EY1out", "EY0dr", "EY1dr"),
...
bb40 %>%
  mutate(`T` = ifelse(grepl(pattern = "0", estimator), 0, 1),
         ...
         n = 1000,
         sdp = sd / sqrt(n),
         z = abs(mean - h0) / sdp,  # how many sd from the mean
         Pvalcheck = 2 * (1 - pnorm(q = z)),
         Pvalcheck = round(Pvalcheck, 3)) %>%
  select(-sdp, -z) %>%
  relocate(estimator, `T`, h0)
```


and the simulation with 100 covariates


```{r}
message("This takes about 3 min. Load from file.")
# startTime <- Sys.time()
# We only use 500 rather than the 1000 suggested in the book
# IMPORTANT: Will be used again whaen calculating p-value
# It gives the same results with actually less sd for EY1exp and EY1dr
# mc.out100 <- mc_standdr(ss = c(100), nrep = 500)
a_file <- file.path(dir_data, "chap06_mc_out100.rds")
mc.out100 <- readRDS(file = a_file)
# saveRDS(mc.out100, file = a_file)
# endTime <- Sys.time()
# print(endTime - startTime)
```
...
and we make a general tableau with the results from both simulations

```{r echo=FALSE}
simdr_tbl <- list()
simdr_tbl <- within(simdr_tbl, {
  df40 <- mc.stats40 %>%
    select(estimator, mean, sd, pval) %>%
    mutate(simulation = "ss=40")
  df100 <- mc.stats100 %>%
    select(estimator, mean, sd, pval) %>%
    mutate(simulation = "ss=100")
  
  title <- "Table 6.13 and 6.14"
  subtitle <- "Sampling Distribution from Simulation"
  subtitle <- paste0(subtitle, "<br>", "Investigating Small-Sample Robustness")
  subtitle <- paste0(subtitle, "<br>", "True E(Y(0))=0.01, True E(Y(1))=0.02")
  df <- rbind(df40, df100) %>%
    mutate(mean = round(mean, 4),
           sd = round(sd, 4),
           pval = round(pval, 2))
  dft <- df %>%
    pivot_longer(cols = c("mean", "sd", "pval")) %>%
    unite(col = "name", simulation, name, sep = "_") %>%
    pivot_wider(id_cols = "estimator", names_from = "name", values_from = "value") %>%
    mutate(`model description` = c("Unadjusted", "Unadjusted",
                           "Linear Exposure", "Linear Exposure",
                           "Logistic Exposure", "Logistic Exposure",
                           "Overspecified Outcome", "Overspecified Outcome",
                           "Doubly Robust", "Doubly Robust")) %>%
    relocate(`model description`, .after = estimator)
  tbl <- gt::gt(dft) %>%
    gt_basic(title = title, subtitle = subtitle) %>%
    tab_spanner_delim(delim = "_", columns = everything(), split = "first") %>%
    cols_align(
      align = "left",
      columns = c("estimator", "model description")
    ) %>%
    tab_style(
...
A plot can however illustrate well Brumback's main points. This ones shows the
estimates' mean with their 5% and 95% quantiles from the simulaiton.


```{r echo=FALSE}
rbind(
  data.frame(simulation = "ss=40", mc.stats40),
  data.frame(simulation = "ss=100", mc.stats100)) %>%
  # mutate(lci = mean - 2 * sd,
  #        uci = mean + 2 * sd) %>%
  mutate(across(.cols = c("mean", "lower", "upper"), .fns = round, digits = 3)) %>%
  ggplot(aes(x = mean, xmin = lower, xmax = upper,
             y = estimator, color = simulation)) +
  geom_pointrange(position = position_dodge(width = 0.5)) +
  ggrepel::geom_text_repel(aes(x = mean, y = estimator, label = mean), size = 3) +
  ggrepel::geom_text_repel(aes(x = lower, y = estimator, label = lower), size = 3) +
  ggrepel::geom_text_repel(aes(x = upper, y = estimator, label = upper), size = 3) +
  theme_minimal() +
  ...
  labs(title = "Chap 6, section 6.3: Simulation of Standardization Methods",
       subtitle =
         sprintf("The mean with 5%% and 95%% quantiles. Sample size = %d.", nrep),
       x = NULL, y = NULL)
```
