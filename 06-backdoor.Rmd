...
source(file = file.path(dir_lib, "fci_06-A_bootstand.R"), 
       local = knitr::knit_global())
source(file = file.path(dir_lib, "fci_06-B_bootstandout.R"), 
       local = knitr::knit_global())
...
```{r echo=FALSE, file="lib\\fci_06-A_bootstand.R"}
...
gt_measures(doublewhatif.out, 
            title = "Table 6.2", 
            subtitle = "Double What-If Study<br>Standardized Estimates with H = AD0")
```
...
gt_measures(doublewhatif.out, 
            title = "Table 6.3", 
            subtitle = "Double What-If Study<br>Standardized Estimates with H = VL0")
```
...
gt_measures(doublewhatif.att, 
            title = "Table 6.5", 
            subtitle = "Double What-If Study<br>Standardized ATT Estimates with H = AD0")
```
...
gt_measures(doublewhatif.att, 
            title = "Table 6.6", 
            subtitle = "Double What-If Study<br>Standardized ATT Estimates with H = VL0")
```

### Standardization with a Parametric Outcome Model

For a the parametric outome model the following script is used

```{r echo=FALSE, file="lib\\fci_06-B_bootstandout.R"}
...
```{r}
whatif2.out <- bootstandout(whatif2dat, formula = vl4 ~ A + lvlcont0)
# whatif2.out
```
...
gt_measures(whatif2.out, 
            title = "Table 6.7", 
            subtitle = "What-If Study<br>Outome Model Standardization with H = lvlcont0")
```
...
```{r}
gssrcc.out <- bootstandout(gssrcc, 
...
gt_measures(whatif2.out, 
            title = "Table 6.8", 
            subtitle = "Effect of <em>More than High School Education</em> on <em>
            Voting for Trump</em><br>Outome Model Standardization")
```

## Standardization via Exposure Modeling


Create the dataset using the mortability data from Table 1.1. The code
is shown in 1.2.1 and again here

```{r echo=TRUE, file="lib\\fci_01-A_mortability.R"}

```

```{r}
mortdat.out <- data_mortability_exp()
mortdat.out
```
...
See previous section for calculation with mortality data
