...
* Positivity assumption of *insured*. Maybe some indiviuals have a zero
...
Now, since *The logistic parametric model is a model for $P(Z = 1 \mid R, H)$*
then
...
coefs <- coef(mod.logit)
PZ1R1 <- plogis(sum(coefs))
PZ1R1
```

and since *the loglinear parametric model is a model for 
$E(Y \mid Z = 0, R, H)$* then
...
dataBRFSS_Z0 <- dataBRFSS
dataBRFSS_Z0$zerodrinks <- 0
dataBRFSS_Z0R1 <- dataBRFSS_Z0
dataBRFSS_Z0R1$rural <- 1
mod.log.predictZ0 <- predict(mod.log, newdata = dataBRFSS_Z0R1, type = "response")
EYZ0R1 <- mean(mod.log.predictZ0)
EYZ0R1
```

and so with the 2-part model we estimate $E(Y \mid R, H)$ as follows

```{r}
maxdrinks.twoparts <- EYZ0R1 * (1 - PZ1R1)
maxdrinks.twoparts
```
 . . . and it is not the correct result ...
