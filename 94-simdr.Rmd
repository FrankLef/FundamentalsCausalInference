...
Finally, we note thet the formula $\beta \cdot \gamma \cdot p + p$ could be 
modified to get $T=600$ by varying the coefficient $\beta=0.13$ and/or $\gamma=20$.
If we change only $\beta$ we could use the result from above and with simple 
algebra
...
and we simulate using $\beta = 0.15$ to validate

```{r}
simdr600.out <- simdr(beta = 0.15, seed = 1009, out_choice = "sim")
simdr600.out$`T`$sum
```
