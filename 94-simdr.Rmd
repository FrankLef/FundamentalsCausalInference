...
and we continue with our reading
...
it dependent on $H$ and which should give $\sum T_i \approx 600$. 
...
We observe that $\sum_i T_i \approx 540$ and therefore $\sum_i T_i \not \approx 600$.
:::
...
Finally, we note that the formula $\alpha \cdot \beta \cdot p + p$ could be 
modified to get $T=600$ by varying the coefficient $\alpha=0.13$ and/or $\beta=20$.
If we change only $\alpha$ and keep $\beta=20$ we could use the result from 
above and with simple algebra
...
```{r}
simdr600.out <- simdr(alpha = 0.15, seed = 1009, out_choice = "sim")
simdr600.out$`T`$sum
```
...
$T=600$ the results are now reasonably close.
