...
```{r}
out <- jack_est(data = dwhatif, func = fciR::instr_linear, conf = 0.95,
...
```{r}
# the list of data with n-1 row
dwhatif.lin <- rsample::loo_cv(dwhatif)

dwhatif.lin$results <- purrr::map_dfr(.x = dwhatif.lin$splits, .f = function(x) {
  df <- analysis(x)
  fciR::instr_linear(df, outcome.name = "VL1", 
                            exposure.name = "A", 
                            instrument.name = "T")
  })
  # select(-`logRR*`) %>%
  # as.data.frame()
str(dwhatif.lin$results)

# dwhatif.lin$out <- data.frame(matrix(ncol = 0, nrow = 0))
out <- purrr::map_dfr(.x = dwhatif.lin$results, .f = ~jack.stats(.))
out
message("the difference in CI is because author uses 1.96 instead of qt()")
```
