...
```{r}
func_instr_vars <- function(data, outcome.name = "Y", exposure.name = "A",
...
```{r}
whatif.est <- func_instr_vars(whatif, outcome.name = "Y", exposure.name = "A", instrument.name = "T")
stopifnot(sum(abs(whatif.est - c(0.007352941, 0.27777778))) < .Machine$double.eps^0.5)
whatif.est
```

and using only 100 boots to estimate the CI we can use `instr_vars` from `fciR`
to 

```{r}
# fciR::instr_vars(whatif, formula = Y ~ A + `T`, exposure.name = "A")
whatif.out <- fciR::boot_est(
  whatif, fciR::instr_vars, times = 100, alpha = 0.05, inv = "none", 
  formula = Y ~ A + `T`, exposure.name = "A", tol = 1e-6)
whatif.out
```
...
and we use a jackknife as discussed above. Since the confidence interval for
the standard error is custom-made, we test it with the `mtcars` data set 
as follows.
...
The functions are found in the `fciR` package as `jack_run` and `jack_est`.
`fciR::jack_run` is the workhorse, just like `fciR::boot_run`. `jack_est` 
compute the inverse just like `boot_est` does.
...
The `jack_est` function to do the full jackknife in one function is included in the 
`fci` package and can be used as follows.
...
# startTime <- Sys.time()
# dwhatif.instr.lin <- fciR::jack_est(data = dwhatif, func = fciR::instr_linear, 
#                               alpha = 0.05, inv = "none", evars = "standard",
#                               VL1 ~ A + `T`, exposure.name = "A")
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap09_dwatif_instr_lin.rds")
# saveRDS(dwhatif.instr.lin, file = a_file)
dwhatif.instr.lin <- readRDS(file = a_file)
dwhatif.instr.lin
```
...
```{r}
message("This takes about 30 sec. Load from file.")
# startTime <- Sys.time()
# dwhatif.instr.loglin <- fciR::jack_est(data = dwhatif, func = fciR::instr_loglinear,
#                               alpha = 0.05, inv = "none", evars = "standard",
#                               VL1 ~ A + `T`, exposure.name = "A")
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap09_dwatif_instr_loglin.rds")
# saveRDS(dwhatif.instr.loglin, file = a_file)
dwhatif.instr.loglin <- readRDS(file = a_file)
...
message("This takes about 30 sec. Load from file.")
# startTime <- Sys.time()
# dwhatif.instr.logit <- fciR::jack_est(data = dwhatif, func = fciR::instr_logistic,
#                               alpha = 0.05, inv = "none", evars = "standard",
#                               VL1 ~ A + `T`, exposure.name = "A")
# endTime <- Sys.time()
# print(endTime - startTime)
a_file <- file.path(dir_data, "chap09_dwatif_instr_logit.rds")
# saveRDS(dwhatif.instr.logit, file = a_file)
dwhatif.instr.logit <- readRDS(file = a_file)
dwhatif.instr.logit
```
...
tbl_9.1 <- rbind(
  ...
  data.frame(
    model = "Logistic",
    dwhatif.instr.logit)) |>
  filter(term %in% c("RD", "logRR", "logOR"))
gt_measures_rowgrp(tbl_9.1,
    rowgroup = "term",
    rowname = "model",
    title = paste("Table 9.1<em>(by FL)</em>", "Double What-If Study", 
                  sep = "<br>"),
    subtitle = paste("Instrumental Variables Analysis",
                     sep = "<br>")
  )
```

The difference in confidence intervals is because the author uses 
`1.96 = round(qnorm(0.95), 2)`
whereas `fciR::jack_est` uses an estimate confidence interval. See `jack_ci`
code for details.
...
gt_measures_rowgrp(
    ...
    rowgroup = "term",
    rowname = "model",
    title = paste("Table 9.1", "Double What-If Study", "<br>"), 
    subtitle = paste("Instrumental Variables Analysis",
                     sep = "<br>"))
```
