...
run `boot()`, alias `meas_effect_modif()` with the RECOVERY data set
...
recovery.out <- boot_est(data = recovery, func = meas_effect_modif,
                         R = 100, conf = 0.95,
                         inv = "exp", vars = vars,
...
ids <- match(paste(bb$estimator, bb$group, sep = "."), recovery.out$name)
comp <- data.frame(bb = bb,
                   d = recovery.out[ids, "est"])
...
recovery.out <- recovery.out %>%
  separate(col = "name", into = c("estimator", "group"), sep = "[.]", remove = TRUE)
...
```{r}
# nces.out <- fciR::meas_effect_modif(nces, formula = highmathsat ~ female + selective)
vars <- c("RR.M0" = "logRR.M0", "RR.M1" = "logRR.M1", "RR.diff"  = "logRR.diff",
            "RR*.M0"  = "logRR*.M0", "RR*.M1"  = "logRR*.M1", "RR*.diff" = "logRR*.diff",
            "OR.M0" = "logOR.M0", "OR.M1" = "logOR.M1", "OR.diff" = "logOR.diff")
nces.out <- boot_est(data = nces, func = meas_effect_modif,
                         R = 100, conf = 0.95,
                         inv = "exp", vars = vars,
                         outcome.name = "highmathsat", exposure.name = "female",
                         modifier.name = "selective")
```
...
ids <- match(paste(bb$estimator, bb$group, sep = "."), nces.out$name)
comp <- data.frame(bb = bb, d = nces.out[ids, "est"])
comp$dev <- abs(comp$bb.est - comp$d)
stopifnot(sum(comp$dev) < 0.01)
```
